...
MongoClient.connect(url, function(err, db) {
  if (err) throw err;
  var dbo = db.db("mydb");
  dbo.createCollection("Users6", function(err, res) {
  if (err) throw err;
...
function receivedMessage(event) {
  ...
  if (messageText) {
    ...
    var query2 = {RECIPIENTID:"to be assigned"}
    dbo.collection("Users6").find(query1).toArray(function(err, result) {
    ...
    dbo1.collection("Users6").insertOne(myobj, function(err, res) {
    if (err) throw err;
    ...
    dbo2.collection("Users6").updateOne(query1, newvalues, function(err, res) {
    if (err) throw err;
   ...
   dbo3.collection("Users6").find(query2).toArray(function(err, result) {
    console.log("i am here now");
    if (err) throw err;  
    

     interval = setInterval(function(){checkPartnerAvailable(result,senderID)},3000);   
    ...
    //db.close();
     ...
     break;
      ...
      dbo.collection("Users6").updateOne(query, newvalues, function(err, res) {
      if (err) throw err;
      //console.log(res);
      sendTextMessage(senderID, "Your session has ended. Type start to begin chat:)");
      //db.close();
        });
      }); 
     ...
     break;
      ...
      dbo.collection("Users6").find(query).toArray(function(err, result) {
      if (err) throw err;
      console.log(result);
      //db.close();
      ...
      sendTextMessage(senderID, "Click on start chat to start a conversation.");
    ...
    //db.close();
      ...
      sendTextMessage(senderID, "Please click on start chat to find a partner");
...
function checkPartnerAvailable(result,senderID){
...
console.log(senderID);
  ...
  for(i=0;i<count;i++){
      ...
      if(result[i].SENDERID!=senderID) {
      console.log('hiiiii');
      console.log("Hi whats up2e");

      myStopFunction(interval);
    

      a2=result[i].SENDERID;
    ...
    var dbo5 = db.db("mydb");
    var query1 = {SENDERID: senderID};
    var newvalues1 = { $set: {SENDERID: senderID, RECIPIENTID: a2} };
    var query3 = {SENDERID: a2};
    var newvalues2 = { $set: {SENDERID: a2, RECIPIENTID: senderID} };
    dbo4.collection("Users6").updateOne(query1, newvalues1, function(err, res) {
    if (err) throw err;
    console.log("1 document updated");
    //db.close();
    });

    dbo5.collection("Users6").updateOne(query3, newvalues2, function(err, res) {
    if (err) throw err;
    console.log("1 document updated");
    db.close();
    });
    ...
    //db.close();
    sendTextMessage(senderID, "You have been matched. Start chatting!" );
    sendTextMessage(a2, "You have been matched!");
    break;   
